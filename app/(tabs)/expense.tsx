...
export default function AddExpense() {
  // Ambil getFinancialSummary dari context untuk mengecek sisa saldo
  const { addTransaction, getFinancialSummary } = useFinance();
  const router = useRouter();
  const [displayNominal, setDisplayNominal] = useState("");
  const [rawNominal, setRawNominal] = useState("");
  const [keterangan, setKeterangan] = useState("");
  const [kategori, setKategori] = useState("Makan & Minum");
  ...
  const handleSave = async () => {
    ...
    if (!finalCleanNumber) {
      Alert.alert("Form Belum Lengkap ⚠️", "Nominal pengeluaran harus diisi!");
      return;
    }
    ...
    // 🛡️ LOGIKA BARU: CEK JIKA PENGELUARAN MELEBIHI SALDO UTAMA
    const currentSummary = getFinancialSummary
      ? getFinancialSummary()
      : { saldo: 0 };
    const sisaSaldoSaatIni = currentSummary.saldo;

    if (parsedNominal > sisaSaldoSaatIni) {
      Alert.alert(
        "Saldo Tidak Cukup ❌",
        `Pengeluaran (${formatRupiahLokal(parsedNominal)}) melebihi total sisa saldo Anda (${formatRupiahLokal(sisaSaldoSaatIni)}).`,
        [{ text: "Mengerti" }],
      );
      return; // Batalkan proses simpan ke Firebase
    }
    ...
    try {
      await addTransaction({
        ...
        keterangan: keterangan || `Pengeluaran ${kategori}`,
  ...
  // Fungsi pembantu untuk memformat angka ke Rupiah di dalam pop-up Alert
  const formatRupiahLokal = (num: number) => {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.cardForm}>
        <Text style={styles.sectionHeader}>Catat Pengeluaran Baru</Text>
        <Text style={styles.sectionSubtitle}>
          Sistem otomatis menolak transaksi jika sisa saldo Anda tidak mencukupi
        </Text>
        ...
        <View style={styles.inputWrapper}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Contoh: Beli buku cetak & fotokopi tugas"
            placeholderTextColor="#a0a0a0"
            value={keterangan}
...
const styles = StyleSheet.create({
  ...
  catBadgeActive: { backgroundColor: "#e74c3c", borderColor: "#e74c3c" },
